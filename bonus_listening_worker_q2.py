...
Transforms the Common_Name to uppercase and writes to a new CSV file.
...
OUTPUT_FILE = "processed_common_names.csv"

def callback(ch, method, properties, body):
    """Define behavior on getting a message."""
    common_name = body.decode()
    transformed_name = common_name.upper()
    logger.info(f"Processed Common Name: {transformed_name}")

    # Write the original and transformed data to a CSV file
    with open(OUTPUT_FILE, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([common_name, transformed_name])
